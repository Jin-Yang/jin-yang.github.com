...
很多的一些插件依赖一些高版本的特性，所以需要手动编译安装，直接从 [www.vim.org/download](https://www.vim.org/download.php) 或者 [github](https://github.com/vim/vim) 上下载相关的版本，如果下载慢，也可以选择镜像文件仓库。
...
----- 可以通过GIT更新源码，也可以下载源码包
$ git clone https://github.com/vim/vim.git

----- 切换到源码目录，注意与如下的VIMRUNTIMEDIR保持一致
$ cd vim81
$ ./configure -h
$ ./configure --with-features=huge                      \
    --enable-cscope                                     \
    --prefix=/usr                                       \ 默认安装到/usr/local目录下
    ...
    --enable-perlinterp                                 \
    --enable-luainterp                                  \
    --enable-gui=no --disable-xim --without-x           \ 关闭GUI
    --enable-fontset               也可以使用gnome或者gtk2
...
如果在编译之后无法找到基础库，例如 `syntax/syntax.vim` ，那么可以在 `make` 时添加环境变量，也就是通过命令 `VIMRUNTIMEDIR=/usr/share/vim/vim81 make` 编译。

此时会安装到 `/usr/bin` 目录下，也就是 `/usr/bin/vim` ，另外，上述的配置选项关闭了 GUI 的支持，也可以通过 `--enable-gui=auto --enable-xim  --with-x` 选项开启。

#### Python 支持

也即是支持对 Python 编写插件的支持，上述支持的是 Python2 ，如果要支持 Python3 需要添加 `--enable-python3interp` 配置项，同时添加相关的 `--with-python-config-dir` 配置，示例如下。
