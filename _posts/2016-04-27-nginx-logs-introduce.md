...
简单来说，当多个进程同时写入一个文件时，会不会导致数据混乱。最常见的就是日志的写入，尤其是对于一些多线程或者多进程的应用来说。

一般小于 `PIPE_BUF` 大小的写入应该是原子的，该宏至少是 512 字节，在 Linux 平台上为 4096 。当然，这里说的是满足 POSIX 标准的文件系统，像 NFS 就不满足这一条件。

严格来说，这与具体的 OS 以及文件系统相关，需要两者的配合才能满足原子操作这一特性，例如 Linux+ext3 是 4096 ，而 Windows+NTFS 是 1024 。
